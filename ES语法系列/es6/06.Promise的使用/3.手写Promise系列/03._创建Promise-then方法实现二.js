...
class wxPromise{
    constructor(/*立即回调函数*/executor){
        ...
        const resolve = (value)=>{
            // 改变 Promise 的状态
            if(this.status === statusArray.pending){ // 这个判断是，防止resolve和reject重复调用时，状态被后一个给覆盖了，只有时在pending时该改状态
...
new wxPromise((resolve,reject)=>{
     ...
     * 
     *  经过 this.status === statusArray.pending 的判断，this.status的状态不会重复改变，只会调用第一个方法
     *
     */
    resolve('1111')
    reject('2222')
