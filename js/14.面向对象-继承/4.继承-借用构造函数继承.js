...
Person.prototype.title = '我是添加的方法';
...
console.log("--子类所以的属性---", stu)   // 可以拿到父类的name

console.log("name---", stu.name)     // 可以拿到父类的name

stu.eating('调用时传参');  // 调用父类的方法

//stu.run('调用时传参');  // 调用父类原型上方法
 ...
 *  借用构造函数存在的弊端 
 * 
 *  1.子类实例化的时候，父类对象会额外的加一些没有必要的属性，因为每个子类拥有的属性或者方法不一样,无法做到定制化
 * 
 *  2.子类不能继承父类原型上的属性和方法
 * 
*/